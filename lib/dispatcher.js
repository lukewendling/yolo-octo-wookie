...
Dispatcher.prototype.start = function() {
  console.log('Dispatcher starting on port ' + this.config.port);
...
Dispatcher.prototype.receive = function(requests) {
  ...
  args.forEach(function(req) {
    JsonSocket.sendSingleMessageAndReceive(self.config.port,
      self.config.host, req, function(err, message) {
      if (err) {
